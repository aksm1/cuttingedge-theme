document.addEventListener('DOMContentLoaded', function () { 
    ...
    // 2. MRCS QBANK ENGINE
    // =================================================================== 
    const appRoot = document.getElementById('mrcs_qbank_app'); 
    let revealedAnswers = {}; 

    if (typeof myQuestionBank !== 'undefined' && appRoot) { 

        const allQuizQuestions = myQuestionBank.questions; // Metadata Only
        const topicTree = myQuestionBank.topic_tree || []; 
        ...
        const storageKey = 'mrcs_quiz_state_' + currentUserID; 
...
<div class="xp-rules-text">
  ...
  <div class="xp-rules-sub">
    ...
    <span>❌ Incorrect: <strong>+1 XP</strong></span>
                ...
                <h3 style="margin-top:30px; margin-bottom:15px; font-size:0.9rem; color:#888; text-transform:uppercase; letter-spacing:1px; font-weight:700;">Other Modes</h3><div id="btnSmartQuiz"><span>🧠</span><div><h3>Smart Quiz</h3><p>AI-picked questions based on your weak areas.</p></div></div><div class="secondary-menu-grid"><div id="btnMock"><span>⏱️</span><div><h3>Mock Exam</h3><p>Simulate exam conditions. 50 Qs, Timed.</p></div></div><div id="btnRandom"><span>🎲</span><div><h3>Random</h3><p>Generate a quiz from the entire question bank.</p></div></div><div id="btnBetweenCases"><span>☕</span><div><h3>Between Cases</h3><p>Generate 10 random questions. Perfect for free time between cases. Doesn't affect saved progress.</p></div></div></div><div id="resumeContainer"></div><div style="margin-top: 30px; text-align:center;"><button id="btnResetAll" style="background:none; border:none; color:#999; text-decoration:underline; cursor:pointer; font-size:0.8rem;">Reset All Progress</button></div>`; 
            menuContainer.innerHTML = html; 

            const listContainer = document.getElementById('dashboardTopicList'); 
            if(topicTree && topicTree.length > 0) { topicTree.forEach(node => { 
                    ...
                    <div class="topic-actions"><button class="btn-topic-start"><span>▶ Start</span></button><div class="btn-topic-subs ${hasChildren ? '' : 'topic-drill-disabled'}">${hasChildren ? 'Choose a Subtopic' : 'No Subs'}</div></div>`; 
        ...
        function resetAllProgress() { const fd = new URLSearchParams(); fd.append('action', 'mrcs_reset_all_history'); fetch(myQuestionBank.ajax_url, { method: 'POST', body: fd }).then(r => r.json()).then(res => { if(res.success) { seenIDs.length = 0; wrongIDs.length = 0; localStorage.removeItem(storageKey); alert("Progress reset."); renderMainMenu(); } }); } 
        ...
        function startSmartQuiz() {
            ...
            if(quizTitleHeader) quizTitleHeader.style.display = 'none';
            if(topPrev) topPrev.style.display = 'none';
            if(topNext) topNext.style.display = 'none';
            quizContainer.innerHTML = `<div style="text-align:center; padding:40px 20px;"><div style="font-size:3rem; margin-bottom:20px;">🧠</div><h2 style="margin-bottom:20px; color:#2c3e50;">Smart Quiz Algorithm</h2><p style="max-width:600px; margin:0 auto 30px; font-size:1.1rem; line-height:1.6; color:#555;">Our Smart Quiz algorithm analyzes your past performance to create a personalized 100-question paper. It prioritizes:<ul style="text-align:left; max-width:400px; margin:20px auto; color:#555;"><li><strong>Unattempted Topics:</strong> Areas you haven't covered yet.</li><li><strong>Weak Areas:</strong> Topics with low accuracy scores.</li><li><strong>Missed Questions:</strong> Specific questions you got wrong previously.</li></ul>This ensures you spend time where it matters most.</p><button id="btnTriggerSmartGen" style="background:#0078D7; color:white; border:none; padding:15px 40px; font-size:1.2rem; border-radius:50px; cursor:pointer; font-weight:bold; box-shadow:0 4px 15px rgba(0,120,215,0.3); transition:transform 0.2s;">Generate Quiz</button><br><br><button id="btnCancelSmart" style="background:none; border:none; color:#999; text-decoration:underline; cursor:pointer;">Cancel</button></div>`;
            
            document.getElementById('btnCancelSmart').onclick = () => { toggleUI('menu'); renderMainMenu(); };
            document.getElementById('btnTriggerSmartGen').onclick = () => {
                quizContainer.innerHTML = '<div style="text-align:center; padding:60px 20px;"><div class="spinner" style="border:4px solid #f3f3f3; border-top:4px solid #3498db; border-radius:50%; width:50px; height:50px; animation:spin 1s linear infinite; margin:0 auto 30px;"></div><h3 style="color:#2c3e50;">Analyzing Performance...</h3><p id="smartLoadingText" style="color:#777;">Checking topic mastery...</p></div><style>@keyframes spin {0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); }}</style>';
                
                const texts = ["Checking topic mastery...", "Identifying weak spots...", "Prioritizing unseen questions...", "Compiling your custom paper..."];
                let step = 0;
                const textInterval = setInterval(() => { step++; if(step < texts.length) { const el = document.getElementById('smartLoadingText'); if(el) el.textContent = texts[step]; } }, 2500);

                setTimeout(() => {
                    clearInterval(textInterval);
                    const fd = new URLSearchParams(); fd.append('action', 'mrcs_generate_smart_quiz');
                    fetch(myQuestionBank.ajax_url, { method: 'POST', body: fd }).then(r => r.json()).then(res => {
                        if (res.success && res.data && res.data.length > 0) { const stubs = res.data.map(id => ({ id: parseInt(id) })); startQuiz(stubs, false, 'Smart Quiz (100 Qs)'); } 
                        else { alert(res.data || "Could not generate quiz."); renderMainMenu(); }
                    }).catch(e => { console.error(e); alert("Error generating quiz."); renderMainMenu(); });
                }, 10000);
            };
        }
        ...
        function startQuiz(q_stubs, exam, titleText, isQuick = false) { 
            ...
            if(quizTitleHeader) { quizTitleHeader.textContent = currentDisplayTitle; quizTitleHeader.style.display = 'block'; }
        ...
        function saveQuizState() { const s = { shuffledIDs: shuffledQuestions.map(q=>q.id), currentQuestionIndex, userAnswers, inFeedbackMode, isExamMode, timeRemaining, sessionID, flaggedIndices, displayTitle: currentDisplayTitle, revealedAnswers }; if (isQuickMode) { localStorage.setItem('mrcs_temp_quick_session', JSON.stringify(s)); } else { localStorage.setItem(storageKey, JSON.stringify(s)); const fd = new URLSearchParams(); fd.append('action', 'mrcs_save_session'); fd.append('data', JSON.stringify(s)); fetch(myQuestionBank.ajax_url, { method: 'POST', body: fd, keepalive: true }); } } 
        function loadQuizState() { if(myQuestionBank.saved_session) return typeof myQuestionBank.saved_session === 'string' ? JSON.parse(myQuestionBank.saved_session) : myQuestionBank.saved_session; const ls = localStorage.getItem(storageKey); return ls ? JSON.parse(ls) : null; } 
        function clearQuizState() { if (isQuickMode) { localStorage.removeItem('mrcs_temp_quick_session'); } else { localStorage.removeItem(storageKey); const fd = new URLSearchParams(); fd.append('action', 'mrcs_clear_session'); fetch(myQuestionBank.ajax_url, {method:'POST', body:fd}); } } 
